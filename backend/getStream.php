<?php
// Securely fetch the M3U8 URL from the backend
$streams = [
    "1" => "https://amg01269-amg01269c1-willowtv-us-5479.playouts.now.amagi.tv/playlist/amg01269-willowtvfast-willowplus-_5859480/index.m3u8",
    "2" => "https://your-secure-m3u8-url-2.com/playlist.m3u8",
    "3" => "https://your-secure-m3u8-url-3.com/playlist.m3u8"
];

$streamId = $_GET['id'] ?? null;

if (!$streamId || !isset($streams[$streamId])) {
    echo json_encode(["error" => "Invalid stream ID"]);
    exit;
}

// Return the M3U8 URL as JSON
echo json_encode(["url" => $streams[$streamId]]);
?>
